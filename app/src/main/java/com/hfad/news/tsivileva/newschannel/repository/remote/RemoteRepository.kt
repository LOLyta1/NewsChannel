...
class RemoteRepository {
   ...
   val PROGER_URL = "https://tproger.ru/feed/"
    ...
    private fun createRetrofit(baseUrl: String, type: FactoryTypes): Retrofit {
     ...
     fun createObservableHabrItem(url:String) : Single<HabrItemsInfo> {
        return createRetrofit(url, FactoryTypes.JSPOON)
                .create(INetwork::class.java)
                .loadHabrDetails()
                .observeOn(Schedulers.io())
                .subscribeOn(AndroidSchedulers.mainThread())
     ...
     fun createObservableProgerItem(url: String):Single<TProgerItemsInfo>{
        return createRetrofit(url, FactoryTypes.JSPOON)
                .create(INetwork::class.java)
                .loadProgDetails()
                .observeOn(Schedulers.io())
