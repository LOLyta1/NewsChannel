    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        view.dialog_sort_feed_button.setOnClickListener {
            when(view.sort_feeds_radio_group.checkedRadioButtonId){
                R.id.sort_feeds_by_abc_asc_radio_button-> listener?.onSortClick(Sort.BY_ABC_ASC)
                R.id.sort_feeds_by_abs_desc_radio_button->listener?.onSortClick(Sort.BY_ABC_DESC)
                R.id.sort_feeds_by_date_asc_radio_button->listener?.onSortClick(Sort.BY_DATE_ASC)
                R.id.sort_feeds_by_date_desc_radio_button->listener?.onSortClick(Sort.BY_DATE_DESC)
            }
            dismiss()
        }
